...
@testset "Constraint-based interactions" begin
    ...
    @testset "compute_eu151_interactions" begin
        ...
        c_total = compute_c_total(Eu151; N_atoms=50_000, omega_ref=omega)
        @test ip[0] + 36 * ip[1] ≈ c_total rtol=1e-12
        @test ip[1] / ip[0] ≈ 1.0/36 rtol=1e-12
    ...
    @testset "_dict_to_delta_gS (Dict input)" begin
        g = SpinorBEC._dict_to_delta_gS(2, Dict(4 => 5.0))
        @test !isempty(g)

        g_empty = SpinorBEC._dict_to_delta_gS(2, Dict{Int, Float64}())
        @test isempty(g_empty)

        # Odd-rank c_3 entry raises ArgumentError.
        @test_throws ArgumentError SpinorBEC._dict_to_delta_gS(2, Dict(3 => 3.0))
    end
    ...
    @testset "interaction_params_from_constraint with c_extra" begin
        ip = interaction_params_from_constraint(; c_total=4689.0, c1_ratio=1.0/36, F=6,
            c_extra=Dict(4 => 50.0))
        @test ip[0] + 36 * ip[1] ≈ 4689.0 rtol=1e-12
        @test ip[4] ≈ 50.0
    end
    ...
    @testset "YAML c_total with c_extra" begin
        ...
        @test ip[4] == 50.0       # c4
        @test ip[6] == -20.0      # c6
        @test ip[2] == 0.0        # c2 unset → zero (Dict default)
    end
