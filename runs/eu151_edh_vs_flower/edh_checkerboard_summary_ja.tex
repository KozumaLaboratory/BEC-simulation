...
であり(定量フィットで $T=36.9\pm0.3\ms$ pure parabolic、$39.3\pm0.4\ms$ quench+parabolic)、
裸のラーモア周期 ($24\ms$) とは異なりかつ振幅依存であること、を示す。
\end{abstract}
...
同一のフィット周期（pure $36.9\pm0.3\ms$、quench $39.3\pm0.4\ms$）で振動し位相のみ異なる
--- 単一の集団スピン--軌道モード。$24\ms$ の裸ラーモアとは別物。}
\label{fig:prec}
\end{figure}
...
\begin{enumerate}\itemsep2pt
  ...
  \item この反転はコヒーレントな spin$\leftrightarrow$orbital 集団モード(定量フィットで
        $T=36.9\pm0.3\ms$ pure / $39.3\pm0.4\ms$ quench、振幅依存性も確認)の一面であり、
        $\Jz$ は厳密に保存される。裸ラーモアでなく 接触$c_1$・DDI・二次ゼーマンにドレスされている。
