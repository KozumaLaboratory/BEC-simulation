...
mode at $T=38\pm2\ms$, distinct from the bare Larmor period ($24\ms$). The
oscillation is elliptical rather than a circular precession, and a third (gentle,
$130\ms$) landing shows the red-shift is protocol-independent.
\end{abstract}
...
$\Lz$ oscillation. We extract the period quantitatively: a nonlinear least-squares fit of a
linear-amplitude-drift sinusoid $A(t)\sin(2\pi t/T+\phi)+C$ to the
\emph{hold-phase only} data (pure: $t\in[90,148]\,\wref$, $n{=}59$; quench:
$t\in[24,80]\,\wref$, $n{=}57$; ramp segments excluded as non-stationary/chirped),
cross-checked against an independent FFT peak estimate:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
against $T_{\mathrm{Larmor}}(26\uG)=24\ms$. Within each protocol, $T_{cb}$, $T_{\Fz}$
and $T_{\Lz}$ agree --- the checkerboard inversion, the orbital $\Lz$ oscillation and
the longitudinal $\Fz$ oscillation are three faces of a \emph{single coherent
spin--orbital collective mode}, and it is clearly \emph{not} the bare Larmor period
($24\ms$).

\paragraph{Two corrections to the numbers above (2026-07-27).} \emph{(i) The quoted
$\pm0.3\ms$ is too small.} It is the formal error of a fit that assumes a pure
sinusoid plus stationary noise. Counting extrema and zero crossings directly, with no
model, the cycle-to-cycle scatter is $\pm1$--$3\ms$; the honest statement is
$T=38\pm2\ms$ ($26\pm2$~Hz). The cleanest single measurement is the quench+parabolic
$\Fz$, whose maxima ($46.8\to85.1\ms$) and minima ($65.6\to104.0\ms$) give $38.3$ and
$38.4\ms$ --- two independent reads agreeing to $0.1\ms$. \emph{(ii) The amplitude
dependence claimed here is not established.} Adding a third protocol (a gentle
$130\ms$ landing) breaks the trend: the pure quench and quench+parabolic have almost
equal amplitude ($0.85$ vs $0.82$ peak-to-peak in $\Fz$) yet differ by $4$~Hz, and the
gentle landing has the smallest amplitude but an intermediate frequency. The three
protocols vary amplitude, hold length and mode content simultaneously, so amplitude
cannot be isolated from them; a scan that varies only the drive strength is required.

\paragraph{It is not a precession.} The word ``precession'' is used loosely above and
should not be read literally. Decomposing the complex transverse field
$\psi_\perp=\Fx+i\langle F_y\rangle$ into $e^{+i\omega t}$ and $e^{-i\omega t}$ gives
comparable weight to both ($|P|/|Q|\sim2$--$3$, i.e.\ a rotation fraction of only
$+0.3$--$0.6$), and the overlap of the texture with itself does not wind in phase. A
rigid rotation about $\hat z$ would put all the weight on one sign and wind by
$2\pi$ per period. What the data show is an \emph{elliptical, largely linear
oscillation} of a spin-vortex texture, not a circular precession.

\begin{figure}[h]\centering
\includegraphics[width=0.75\linewidth]{edh_precession_vs_L.png}
\caption{Checkerboard $cb(t)$, $\Lz$ (orbital) and $\Fz$ (spin) all oscillate at the
same period ($38\pm2\ms$), phase-shifted --- one collective spin--orbital mode,
distinct from the $24\ms$ bare Larmor period. The label ``precession'' on this figure
is loose: the motion is an elliptical, largely linear oscillation (see text).}
\label{fig:prec}
\end{figure}
...
\begin{enumerate}\itemsep2pt
  ...
  \item The inversion is one face of a coherent spin$\leftrightarrow$orbital
        collective mode at $T=38\pm2\ms$, elliptical rather than a circular
        precession, with $\Jz$ conserved to $2\times10^{-4}$; it is red-shifted from
        the bare Larmor period, and the red-shift survives landing speeds from
        $5.8$ to $130\ms$.
