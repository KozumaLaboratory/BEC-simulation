...
mode, quantitatively fit at $T=36.9\pm0.3\ms$ (pure parabolic) and
$39.3\pm0.4\ms$ (quench+parabolic) --- distinct from the bare Larmor period
($24\ms$) and itself amplitude-dependent.
\end{abstract}
...
\end{aligned}
\end{equation}
against $T_{\mathrm{Larmor}}(26\uG)=24\ms$. (The FFT-only peak sits
$\sim15\%$ higher in both cases, consistent with spectral leakage over a
$\lesssim2$-cycle hold window; the time-domain fit is the more reliable estimate
and is quoted above.) Within each protocol, $T_{cb}$, $T_{\Fz}$ and $T_{\Lz}$ agree
to within their sub-ms uncertainties --- the checkerboard inversion, the orbital
$\Lz$ oscillation and the longitudinal $\Fz$ oscillation are three faces of a
\emph{single coherent spin--orbital collective mode}. It is \emph{not} the bare
Larmor precession ($24\ms$): the $\sim1.5$--$1.6\times$ slowdown is the DDI +
quadratic-Zeeman dressing, confirming the dynamics is interaction-driven (EdH), not
single-particle. So both proposed pictures --- ``$\Lz$ oscillation'' and ``spin
precession'' --- are correct simultaneously; they are the same mode.

A second, new result falls out of comparing the two protocols directly: the mode
period itself is \emph{amplitude-dependent}. Quench+parabolic reaches a larger
$|\Fz|,|\Lz|$ excursion (Table~\ref{tab:cmp}) and runs $\sim2.4\ms$ \emph{slower}
than pure parabolic ($36.5\to38.9\ms$, averaging $cb/\Fz/\Lz$) --- a several-$\sigma$
difference given the sub-ms fit uncertainties on each side. This is consistent with
a nonlinear, self-consistent collective-mode frequency (its restoring force comes
from the mean-field DDI/contact energy, which depends on how far the state has been
driven from $m{=}{-}6$), not a fixed linear-response resonance.
...
and $\Fz$ (spin) all oscillate at the same fitted period ($36.9\pm0.3\ms$ pure,
$39.3\pm0.4\ms$ quench), phase-shifted --- one collective spin--orbital mode,
distinct from the $24\ms$ bare Larmor period.}
\label{fig:prec}
\end{figure}
...
\begin{enumerate}\itemsep2pt
  ...
  \item The inversion is one face of a coherent spin$\leftrightarrow$orbital
        collective mode, quantitatively fit at $T=36.9\pm0.3\ms$ (pure) /
        $39.3\pm0.4\ms$ (quench) and shown to be amplitude-dependent, with $\Jz$
        exactly conserved; it is DDI/quadratic-Zeeman-dressed, not bare Larmor.
