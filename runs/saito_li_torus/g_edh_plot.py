...
Reads the CSVs `f_edh.jl` writes. Two panels, matching the paper's Fig. 4(b):
  ...
  python3 runs/saito_li_torus/g_edh_plot.py
"""
import pathlib
import numpy as np
import matplotlib
...
def main():
    ...
    if not files:
        print(f"no edh_*.csv in {OUT} — run f_edh.jl first")
        return
