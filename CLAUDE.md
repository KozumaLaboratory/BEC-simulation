...
- **The combined preconditioner `P_C` (`precond_alpha_v ≥ 0`) is OFF by default because it LOSES** — ~40× worse on the weak-field Eu+DDI soft manifold at 24³ (measured 2026-06-23, `d496dd71`). The 40× is measured; the **mechanism is open**. It was recorded as "a diagonal preconditioner cannot precondition a collective Goldstone", and that explanation is NOT supported: `bench/probe_lbfgs_orbit_fraction.jl` finds the L-BFGS step orthogonal to the orbit tangent to machine precision (median 2.4e-17, max 1.8e-13, against a passing positive control of 1.000000) — the Goldstone is not in the iterate path at all, so it cannot be what P_C fails to fix. That the ground state breaks the exact axial U(1) `e^{-iθ(L_z+F_z)}` is still true; it is the causal step that does not follow. `P_C` is Antoine-Levitt-Tang (arXiv:1611.02045), where preconditioning IS the dominant lever — for trapped, gapped problems. Consequence: on that problem the iteration count is set by a flat direction, so **per-iteration cost is the lever** and quotienting the symmetry is the only thing that would attack the count. The bench kept to stop re-derivation was deleted two days later as a one-off driver (`40c329a5`); this line is the replacement.
