...
- **Divergence kill**: reap loop watches `_live_status.json`, cancels divergent runs, classifies `:killed_data` — **on the standard `dynamics:` path ONLY**. `_step_dispatch!` hands `live_status_path` to `DynamicsStep` and to nothing else, so `BinaryDynamicsStep` and `RotatingBasisDynamicsStep` write no status file and a diverging run on either finishes and bills. Declared, with reasons, in `test/workflow/test_every_dynamics_path_reports_liveness.jl`; a new dynamics kind must wire it or be added there deliberately. (This bullet stated the guarantee unconditionally until 2026-08-06, and the mechanism itself had never fired at all until 2026-08-04 — writer and reader shared no keys.)
