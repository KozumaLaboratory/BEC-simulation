...
- **Odd-rank `c_extra` ignored** by design. Even-rank only, and the channel API is a **`Dict{Int,Float64}` keyed by rank** — `InteractionParams(Dict(0 => c0, 1 => c1, 2 => c2, 4 => c4))`, or the `c_extra` kwarg of `interaction_params_from_constraint` / `eu_preset`. The rank travels next to its value, so the positional `[c2, c4, c6]` misindexing footgun for F ≥ 3 is unrepresentable.
...
- **The rank-keyed `Dict` is the canonical channel API.** `even_c_extra(F; c2, c4, …)` was named here as canonical until 2026-08-19 and has had **zero definitions since `effdc978` (2026-05-25)**, which deleted it when `InteractionParams` moved to `Dict{Int,Float64}` — the Dict made the misindexing it guarded against unrepresentable, so the guard went with it. Two entries in this file outlived the symbol by three months; `src/model/specs.jl` still cited its "documented footgun" as if the doc existed.
