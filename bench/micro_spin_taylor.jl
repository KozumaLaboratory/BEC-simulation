...
function setup(; n=N_GRID, D=D_COMP, T=Float64)
    F = (D - 1) // 2
...
function main()
    dev = CUDA.device()
    @printf("micro spin-taylor — %s, %d^3 × D=%d, Float64\n",
        CUDA.name(dev), N_GRID, D_COMP)
    s = setup()
    scale = 1.0e-3
    ...
    for K in (1, 2, 3, 4, 6, 8, 12, 16, 24)
        t = time_ms(() -> EXT._apply_spin_rotation_taylor!(
            s.psi, s.vx, s.vy, s.vz, s.coef, scale, K, Val(D_COMP)))
    ...
    t4 = time_ms(() -> EXT._apply_spin_rotation_taylor!(
        s.psi, s.vx, s.vy, s.vz, s.coef, scale, 4, Val(D_COMP)))
    t16 = time_ms(() -> EXT._apply_spin_rotation_taylor!(
        s.psi, s.vx, s.vy, s.vz, s.coef, scale, 16, Val(D_COMP)))
