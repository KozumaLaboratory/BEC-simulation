...
- `zeeman:` — Level 0 (`p`/`q`/`bx`/`by`), Level 1 (`Bx`/`By`/`Bz` in Gauss strings), Level 2 (`B_mag`/`theta_deg`/`phi_deg`). Mixing levels in one block raises `ArgumentError`. Ramps via `{from: …, to: …}` are expanded to a `TimeDependentZeeman`.
