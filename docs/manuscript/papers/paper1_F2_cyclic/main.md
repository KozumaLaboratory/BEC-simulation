...
with the channel-coupling decomposition
$c_0 = (4g_2 + 3g_4)/7$, $c_1 = (g_4 - g_2)/7$, $c_2 = (7g_0 - 10g_2 + 3g_4)/7$.

### B. Cyclic ground state spinor

$$\zeta_{\rm cyc} = \frac{1}{2}(1, 0, i\sqrt{2}, 0, 1) \tag{II.2}$$

Properties: $|\zeta|^2 = 1$, $\langle F_z \rangle = 0$, $\langle F_+ \rangle = 0$,
$T_d$ symmetric (4 Majorana points form tetrahedron).

### C. BdG matrix elements

$h_{\rm mf}[m, m'] = \sum_S g_S \cdot$ (CG-weighted ζ products) — full 5×5 matrix
displayed in Appendix A.

Anomalous matrix $M_{\rm anom}[m, m']$ with $m + m' \in \{-4, -2, 0, +2, +4\}$
contributing.

### D. m-parity block decomposition

$h_{\rm mf}$ couples only $m \equiv m' \pmod{2}$ in the cyclic phase.
...
$$P_{\rm odd}(\omega, \varepsilon_k) = \frac{1}{49}\left[7\omega^2 - 7\varepsilon_k^2 + 4n(g_2 - g_4)\varepsilon_k\right]^2 \tag{III.1}$$

Positive-energy modes (2-fold degenerate):
$$\omega^2_{4,5} = \varepsilon_k(\varepsilon_k + 4nc_1) \tag{III.2}$$

### B. Even block (m = ±2, 0)

Characteristic polynomial factorises into four parts:
$$P_{\rm even}(\omega, \varepsilon_k) = \frac{1}{60025}\,P_1(\omega) \cdot P_2(\omega) \cdot Q_1(\omega) \cdot Q_2(\omega) \tag{III.3}$$

where
- $P_1$: phonon factor → $\omega^2_1 = \varepsilon_k(\varepsilon_k + 2nc_0)$ (Eq. III.5)
- $P_2$: spin Goldstone → $\omega^2_2 = \varepsilon_k(\varepsilon_k + 4nc_1)$ (Eq. III.7)
- $Q_{1,2}$: linear in ω, give $\omega_3 = \varepsilon_k + 2nc_2/5$ (Eq. III.9)

### C. Mode summary

| Mode | $\omega^2(\varepsilon_k)$ | $\xi_\lambda$ | $|\Delta_\lambda|$ | Type |
|---|---|---|---|---|
| 1 | $\varepsilon_k(\varepsilon_k + 2nc_0)$ | $c_0$ | $c_0$ | Phonon (U(1) Goldstone) |
| 2 | $\varepsilon_k(\varepsilon_k + 4nc_1)$ | $2c_1$ | $2c_1$ | Spin Goldstone (even) |
| 3 | $(\varepsilon_k + 2nc_2/5)^2$ | $2c_2/5$ | **0** | **Amplitude (cyclic-specific)** |
| 4, 5 | $\varepsilon_k(\varepsilon_k + 4nc_1)$ (×2) | $2c_1$ | $2c_1$ | Spin Goldstones (odd) |

### D. Goldstone counting

Symmetry breaking: SO(3) × U(1) → T_d (order 12)
