...
# nominal
K3n,heatn,taun = 1e-42, 0.05, 15.0
logspace(a,b,n)=10 .^ range(log10(a),log10(b);length=n)
heats=collect(range(0.01,0.30;length=21))
K3s  =collect(logspace(1e-42,1e-40,25))         # full two decades, up to 1e-40
taus =collect(range(5.0,40.0;length=21))
