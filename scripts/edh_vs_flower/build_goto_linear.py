"""goto.h5 for a LINEAR ramp run: F fields from psi13, t from diag, B(t) analytic =
linear 10mG->26uG over TDESC_INT internal, then hold 26uG. env: PSI13, DIAG, OUT, TDESC_INT"""
...
PSI13=os.environ["PSI13"]; DIAG=os.environ["DIAG"]; OUT=os.environ["OUT"]; TDESC=float(os.environ["TDESC_INT"])
OM=691.15; B0=0.01; BEND=2.6e-5
...
if len(t)!=nf: n=min(len(t),nf); t=t[:n]; nf=n
Bg=np.where(t<=TDESC, B0+(BEND-B0)*np.clip(t/TDESC,0,1), BEND).astype(float)
...
for a in range(0,nf,CH):
    ...
    for j,k in enumerate(fr):
        p=ps[j]
        F["Fx_3d"][...,k]=np.real(np.einsum("xyzm,mn,xyzn->xyz",np.conj(p),FX,p)); F["Fy_3d"][...,k]=np.real(np.einsum("xyzm,mn,xyzn->xyz",np.conj(p),FY,p)); F["Fz_3d"][...,k]=np.real(np.einsum("xyzm,mn,xyzn->xyz",np.conj(p),FZ,p))
...
O["t"]=t; O["B_gauss"]=Bg; g=O.create_group("meta")
for k,v in (("F",6),("NX",64),("L_box",18.0),("vol_stride",2),("omega_ref",OM)): g[k]=v
O.close(); print(f"wrote {OUT}: {nf}frames tms {t[0]/OM*1000:.2f}->{t[-1]/OM*1000:.1f} B {Bg[0]*1e6:.0f}->{Bg[-1]*1e6:.0f}uG")
