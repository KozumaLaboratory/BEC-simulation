...
for k in range(nf):
    fig,axs=plt.subplots(1,2,figsize=(9.4,5.2),sharey=True); fig.subplots_adjust(left=0.08,right=0.89,bottom=0.12,top=0.78,wspace=0.06)
    ...
    fig.suptitle(f"EdH 放物線ランプ (par_T90): 差分撮像（傾け − 無傾斜）　t={tms[k]:.0f} ms",fontsize=14,y=0.965)
