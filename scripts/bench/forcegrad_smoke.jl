...
@printf("\nExpected: ForceGrad-4A00 ≈ order 3-4. Strang ≈ 2, Y4-mid ≈ 4.\n")
@printf("Sub-4 order is finite-difference truncation of |∇V|² (O(dx²) central\n")
@printf("diff used here; paper §IV recommends FFT spectral ∇ — deferred).\n")

order_table(50.0, "NONLINEAR — c0=50, GP equation")
@printf("\nExpected: ForceGrad-4A00 order ≈ 1 (4A00 uses ψ(0) MF for all stages →\n")
@printf("dominant MF error from missing midpoint self-consistency). Full 4AWW with\n")
@printf("Picard on V(dt/2) recovers order 4 — deferred to next iteration.\n")
@printf("Strang ≈ 2 (still wins over 4A00 here), Y4-mid ≈ 4 (Track A1 baseline).\n")
