...
# colour mode: "updown" = red(↑z)/blue(↓z) by F_z/|F| (default);
#              "hsv"    = legacy full-direction HSV (azimuth→hue).
SPIN_COLOR = os.environ.get("FPE_SPIN_COLOR", "updown")


def direction_to_updown_rgba(fx, fy, fz, polarisation):
    """Colour a unit ⟨F̂⟩ by its z-projection: +z up = red, −z down = blue
    (RdBu_r on cos θ = F_z/|F| ∈ [−1, 1]). alpha = polarisation so weak
    arrows fade. No azimuthal hue (no rainbow)."""
    fmag = np.sqrt(fx*fx + fy*fy + fz*fz) + 1e-30
    cos_t = np.clip(fz / fmag, -1.0, 1.0)
    rgb = plt.cm.RdBu_r((cos_t + 1.0) / 2.0)[..., :3]
...
def _make_updown_legend(ax_legend):
    """Vertical RdBu_r bar for the up/down colour mode: +z=red, −z=blue."""
    grad = np.linspace(1, -1, 256).reshape(-1, 1)
    ax_legend.imshow(grad, aspect="auto", cmap="RdBu_r",
                     extent=[0, 1, -1, 1], origin="upper")
    ax_legend.set_xticks([])
    ax_legend.set_yticks([-1, 0, 1])
    ax_legend.set_yticklabels(["−1 (↓ down)", "0 (in-plane)", "+1 (↑ up)"], fontsize=8)
    ax_legend.set_title(r"arrow colour: $F_z/|F|$", fontsize=9, pad=4)
...
_make_updown_legend(axL) if SPIN_COLOR == "updown" else _make_color_sphere(axL)
...
def draw_frame(k):
    ...
    _rgba_fn = direction_to_updown_rgba if SPIN_COLOR == "updown" else direction_to_hsv_rgba
    rgba = _rgba_fn(fx_k, fy_k, fz_k, polarisation)
