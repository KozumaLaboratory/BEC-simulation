#!/usr/bin/env python3
"""
3-D spin GIF for the LBFGS B-sweep.

Visualisation:
  - sparse 3-D quiver arrows from the downsampled spin field
  - arrow colour = local Fz / n_total
  - right panel = B sweep trace with the current frame highlighted
"""
import os
import numpy as np
import h5py
...
H5 = os.path.join(ROOT, "b_sweep_3d.h5")
OUT_GIF = os.path.join(ROOT, "b_sweep_3d_spin.gif")

DENSITY_THRESH = float(os.environ.get("FPE_3D_SPIN_THRESH", "0.12"))
ARROW_STEP = int(os.environ.get("FPE_3D_ARROW_STEP", "2"))
FPS = int(os.environ.get("FPE_3D_FPS", "4"))
INTERVAL_MS = int(os.environ.get("FPE_3D_INTERVAL_MS", "250"))
...
print(f"loaded {Nb} B points, 3-D grid={Nv}^3, stride={vol_stride}")
...
axB.plot(np.arange(Nb), B_uG, "C2-", lw=1.7)
axB.set_xlabel("B-sweep step", color=FG)
axB.set_ylabel("B [μG]", color=FG)
axB.set_title("B sweep trace", color=FG, fontsize=12)
axB.tick_params(colors=FG, labelsize=9)
for spine in axB.spines.values():
    spine.set_color("black")
axB.grid(alpha=0.2, which="both", color="black")
vB = axB.axvline(0, color="#ff5c5c", lw=1.2)
...
def draw_frame(k):
    ...
    B = B_uG[k]
    ax3.set_title(
        r"3-D spin texture (LBFGS B-sweep)"
        + f"\n B = {B:+.0f} μG",
