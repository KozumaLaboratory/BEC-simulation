#!/usr/bin/env julia
"""
extract_b_sweep_3d_spin.jl
==========================
Build a compact HDF5 file for B-sweep 3D spin visualization by reading the
per-B LBFGS cache psi states.
...
const OUT = joinpath(ROOT, "b_sweep_3d.h5")
...
function extract_3d_observables(psi, idxs)
    ...
    (; n_total, n_m6, arg_m6, Fx_3d, Fy_3d, Fz_3d)
end
...
function main()
    ...
    nv = length(idxs)
    Nb = length(B_ALL_UG)
    println("[b_sweep_3d] ROOT=$ROOT  vol_stride=$VOL_STRIDE  grid=$nv^3")
    println("[b_sweep_3d] B values = $(B_ALL_UG)")

    # Preflight cache availability.
    ...
    for (i, Btarget) in enumerate(B_ALL_UG)
        ...
        println("[b_sweep_3d] loading $path")
