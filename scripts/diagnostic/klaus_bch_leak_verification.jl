    ...
    try
        jldopen(path, "r") do f
            ...
            if steady_idx === nothing
                error("phi=$phi: no snapshot found with t > $SPINUP_END (spinup end). Check SPINUP_END or times array.")
            end

            n_steady = length(times) - steady_idx + 1
            if n_steady < MIN_STEADY_SNAPSHOTS
                error("phi=$phi: only $n_steady steady snapshots (need $MIN_STEADY_SNAPSHOTS). Check phase boundaries.")
            end
...
primary_confirm = max_norm_drift_global < PRIMARY_CONFIRM_THRESHOLD &&
                  (!isnan(growth_ratio) && growth_ratio < 5.0)
primary_refute  = max_norm_drift_global > PRIMARY_REFUTE_THRESHOLD ||
                  (!isnan(growth_ratio) && growth_ratio > 5.0)

primary_verdict = primary_confirm ? :CONFIRM :
                  primary_refute  ? :REFUTE  : :INCONCLUSIVE
...
secondary_verdict = secondary_confirm ? :CONFIRM :
                    secondary_refute  ? :REFUTE  : :INCONCLUSIVE
