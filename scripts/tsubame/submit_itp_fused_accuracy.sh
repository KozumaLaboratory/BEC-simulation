...
echo "### SMOKE"
$JULIA --project=. bench/itp_fused_chain_accuracy.jl 16 400 2>&1 | grep --line-buffered -vE "$FILT"
rc=${PIPESTATUS[0]}; echo "### smoke rc=$rc"
[ "$rc" -ne 0 ] && { echo "SMOKE FAILED"; echo "ALL DONE $(date)"; exit 1; }
echo; echo "### PRODUCTION 64^3"
$JULIA --project=. bench/itp_fused_chain_accuracy.jl 64 40000 2>&1 | grep --line-buffered -vE "$FILT"
echo "ALL DONE $(date)"
