...
# `<base_dir>/<run_name>/viewspec.json` (produced by `write_viewspec`,
# src/analysis/sweep_viewspec.jl). 404 when the file is absent so the
# frontend can show "no viewspec yet" rather than spinning forever.
...
function _route_sweep(path::String, base_dir::String)
    ...
    isfile(viewspec_path) || return (
        404,
        "application/json",
        "{\"error\":\"No viewspec for $(p.name); generate one with write_viewspec (src/analysis/sweep_viewspec.jl)\"}",
    )
