...
function print_scan_summary(run_dir::String; io::IO = stdout)
    summary = scan_summary(run_dir)
    isempty(summary) && (println(io, "No results found in $run_dir"); return)

    println(io, "── Scan Summary: $(basename(run_dir)) ──")
    println(io, "  $(length(summary)) results from $(length(unique(r.scan_index for r in summary))) scan points")
    println(io)
    ...
    for idx in unique(r.scan_index for r in summary)
        ...
        for r in runs
            ...
            println(io, "    $(rpad(label, 20)) E=$(lpad(string(round(r.energy; sigdigits=8)), 14))  " *
    ...
    if !isempty(comps)
        ...
        for c in comps
            ...
            println(io, "  Point $(c.scan_index): ΔE($(c.run_a)−$(c.run_b)) = $(round(c.ΔE; sigdigits=4))  → $arrow")
        end
    end
end
