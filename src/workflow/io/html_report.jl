...
function generate_html_report(run_dir::AbstractString;
    ...
    println(io, "<p class='muted'>Generated $(now())  •  $(length(files)) point(s)  •  $(length(pngs)) frame(s)</p>")

    println(io, "<h2>Per-point summary</h2>")
    println(io, "<table><tr><th>file</th><th>energy</th><th>conv</th><th>Mz</th><th>analyzers</th></tr>")
    for fname in files
        ...
        try
            jldopen(path, "r") do f
                ...
                println(io, "<tr><td><code>$fname</code></td><td>$e</td><td>$c</td><td>$m</td><td>$analyz</td></tr>")
            end
        catch e
            println(io, "<tr><td><code>$fname</code></td><td colspan='4' class='muted'>read error: $e</td></tr>")
        end
    end
    println(io, "</table>")
    ...
    if !isempty(pngs)
        ...
        println(io, "<p class='muted'>Showing first + last; full sequence: $(length(pngs)) frames in <code>frames/</code></p>")
    end
