# Unified SpinorBEC CLI body. `scripts/cli.jl` is a thin shim that calls
# `cli_main(ARGS)`; everything here is pure dispatch into library functions.
#
# Usage:
...
function _cmd_tag(args)
    ...
    if sub == "add"
        ...
        rec = tag_run!(rest[1], rest[2]; created_by="cli")
...
# ── gs-library subcommand (converged-GS library, src/workflow/io/gs_library.jl) ──

function _cmd_gs_library(args)
    if isempty(args)
        println(stderr, "usage: cli.jl gs-library {index <dir> | merge [src] [dest]}")
        return 2
    end
    sub, rest = args[1], args[2:end]
    if sub == "index"
        isempty(rest) &&
            (println(stderr, "usage: cli.jl gs-library index <scan_dir>"); return 2)
        rows = gs_library(rest[1])
        println("indexed $(length(rows)) GS states → $(rest[1])/library.csv")
        for r in rows
        ...
        return 0
    elseif sub == "merge"
        src = length(rest) >= 1 ? rest[1] : "figs"
        dest = length(rest) >= 2 ? rest[2] : DEFAULT_GS_LIBRARY
        merge_gs_library(src, dest)
        return 0
    end
    println(stderr, "cli.jl gs-library: unknown subcommand '$sub'")
    2
end

# ── validation-matrix subcommand ─────────────────────────────────────

function _cmd_validation_matrix(args)
    out_dir = isempty(args) ? "docs/validation" : args[1]
    s = run_validation_matrix(out_dir)
...
Entry point behind `scripts/cli.jl`: dispatches to the subcommand handlers
