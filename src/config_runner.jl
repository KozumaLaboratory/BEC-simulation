...
function _run_config(config::UnifiedConfig, ::GroundStateExperiment; verbose::Bool=true)
    ...
    verbose && println("Ground state ($(config.ground_state.n_steps) steps, tol=$(config.ground_state.tol))...")

    result = _run_ground_state(config, grid, atom, potential, ndim)

    verbose && println("  converged=$(result.converged), E=$(result.energy)")

    (ground_state_energy=result.energy, ground_state_converged=result.converged,
     psi=copy(result.workspace.state.psi), workspace=result.workspace)
end
...
function _run_config(config::UnifiedConfig, spec::DynamicsExperiment; verbose::Bool=true)
    ...
    phase_results, phase_names = _run_dynamics_sequence(
        config, spec, grid, atom, ndim, psi_current; verbose)

    (ground_state_energy=gs_result.energy, ground_state_converged=gs_result.converged,
     phase_results=phase_results, phase_names=phase_names)
end
...
function _run_dynamics_sequence(config::UnifiedConfig, spec::DynamicsExperiment,
                                grid, atom, ndim, psi_current; verbose::Bool=true)
    sys = config.system
    ...
    for (i, phase) in enumerate(spec.sequence)
        ...
        sim_result = if phase.integrator.method == :adaptive
            ...
            verbose && println("  adaptive: $(out.n_accepted) accepted, $(out.n_rejected) rejected")
            out.result
        elseif phase.integrator.method == :yoshida
            ...
            verbose && println("  yoshida: $(out.n_accepted) accepted, $(out.n_rejected) rejected")
            out.result
        else
            run_simulation!(ws)
        end
