# Vega-Lite viewspec builder: resolves a `SweepResult` into a Vega-Lite
# spec (a Dict; serialise with JSON.print). The dispatcher picks the
# view shape and the primary inference mark (asymptote / collapse /
# residual / scaling / correlation) from the `Hypothesis`; renderers
# paint the spec, they do not decide. Consumes the contract, colormaps,
# and `compute_quality_alpha` defined in the sibling sweep_* files.

using JSON

export to_viewspec, write_viewspec
