# Sweep result contract types — the declared structure of a parameter sweep.
#
# `SweepResult` is the tidy carrier consumed by both the dashboard `to_viewspec`
# pipeline and Makie `plot_sweep`. `SweepAxis` / `SweepObservable` declare the
# sweep's shape (note `m` is NOT an axis — it lives inside a spectrum-kind
# observable); `Hypothesis` / `ModelSpec` carry the question + theoretical
# apparatus. `dominant_m_with_margin` is the margin-based dominant-m extractor
# (NOT a purity threshold — see docs/architecture/sweep_view.md for the failure
# mode an absolute threshold has on 2-state 50-50 cells).
