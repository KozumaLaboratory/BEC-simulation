# Sweep → Vega-Lite viewspec dispatcher.
#
# `to_viewspec` resolves a `SweepResult` into a Vega-Lite spec (returned as a
# Dict for JSON serialisation): it picks the view_shape from the swept-axis
# count, resolves per-cell fill / quality_alpha / edges, and emits marks.
# Renderers paint the spec; they do not decide. `_cell_edges` computes the
# axis-aware cell boundaries; `write_viewspec` serialises the spec to JSON.
