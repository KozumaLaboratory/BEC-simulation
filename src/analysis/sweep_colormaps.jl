# Reference colormap LUTs + per-cell hex resolution (frozen for
# reproducibility). cmocean `balance` for signed observables, viridis for
# positive/monotone ones. Computed once and frozen so future versions can
# pin `colormap_version: "1.0"` for reproducibility gates.
...
# Pack an 8-bit RGB triple as a `#RRGGBB` hex string.
_rgb_to_hex(r::Integer, g::Integer, b::Integer) =
    string("#",
...
_lut_hex_array(lut::Vector{NTuple{3, UInt8}}) =
    [_rgb_to_hex(r, g, b) for (r, g, b) in lut]
...
function _positive_clip_range(result::SweepResult, obs_key::Symbol)
    ...
    for r in result.data
        ok = conv_col === nothing || (haskey(r, conv_col) && Bool(r[conv_col]))
        if ok
            v = get(r, obs_key, NaN)
