...
# three-body loss rate it produces in the atoms-lost convention dN = −K₃⟨n²⟩N, where K₃ ≡ the
# thesis's Söding L₃ (atom-loss coefficient). For a TF condensate the density-weighted mean-square
# density is ⟨n²⟩ = ∫n³/∫n = (8/21) n₀² (NOT 4/7 n₀², which is ⟨n⟩/n₀ — a different moment).
# Matches the thermal-cloud convention in `_thermal_evap_rates` (⟨n²⟩ = n_pk²/3^{3/2}).
