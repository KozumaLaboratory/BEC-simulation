...
export matsui_fig4b_report

# Overridable (MATSUI_FIXTURES env) so the report can run from a scratch copy
# on a cluster login node without checking out the repo a second time.
...
point that looks like a completely untransferred cloud, which at the edge of a
scan is exactly where the baseline of a dip measurement comes from.

The cost of using the series instead is that the last sample lands on the last
multiple of `save.every`, so make `save.every` divide the step count (3456 =
32 x 108). This function checks it did, and refuses rather than quietly
reporting a population from 98 % of the hold.
"""
function _matsui_final_populations(run_dir; duration=nothing)
    ...
    map(files) do f
        jldopen(joinpath(run_dir, f), "r") do d
            ...
            # One dt of slack: the integrator takes ceil(duration/dt) steps, so
            # the last sample lands at 3.4560 for a nominal 3.4558. Anything
            # larger means save.every does not divide the step count.
...
function _matsui_reference_dip(lo, hi)
    lines = readlines(joinpath(_matsui_fixdir(), "dataset_fig4_theo.csv"))
    B = Float64[]
    N6 = Float64[]
    for ln in lines[2:end]   # skip header
        c = split(ln, ',')
        length(c) >= 2 || continue
        push!(B, parse(Float64, c[1]))
        push!(N6, parse(Float64, c[2]))
    end
    p = sortperm(B)
    B, N6 = B[p], N6[p]
    keep = (B .>= lo) .& (B .<= hi)
...
"""
    matsui_fig4b_report(run_dir; out_csv=nothing, io=stdout) -> (; run, ref)

Report the N_{m=-6} dip centre / half-depth width of a `runs/matsui_fig4b`
B-scan against the published Matsui simulation restricted to the same field
window. Optionally writes the per-field population table to `out_csv`.
"""
function matsui_fig4b_report(run_dir::AbstractString;
    ...
    println(io, "field [nT]   N_{-6}      frac      Fz/N")
    for (b, p) in zip(B, pops)
        fz = sum((F - (c - 1)) * p[c] for c in 1:D) / sum(p)
        @printf(io, "%9.2f  %10.1f  %8.5f  %8.4f\n", b, N_atoms * p[D], p[D], fz)
    end

    d = resonance_dip(B, N6)
    (ref, nref) = _matsui_reference_dip(minimum(B), maximum(B))

    println(io)
    @printf(io, "%-28s %12s %12s\n", "", "centre [nT]", "width [nT]")
    @printf(io, "%-28s %12.4f %12.4f\n", "SpinorBEC (this run)", d.center, d.width)
    @printf(io, "%-28s %12.4f %12.4f   (%d pts in window)\n",
        "Matsui simulation, same window", ref.center, ref.width, nref)
    @printf(io, "%-28s %12.4f %12.4f\n", "difference", d.center - ref.center,
        d.width - ref.width)

    if out_csv !== nothing
        open(out_csv, "w") do f
            println(f, "B_nT," * join(["N_m$(F - (c - 1))" for c in 1:D], ","))
            for (b, p) in zip(B, pops)
                println(f, join([string(b); [string(N_atoms * x) for x in p]], ","))
            end
        end
        println(io, "\nwrote $out_csv")
    end
